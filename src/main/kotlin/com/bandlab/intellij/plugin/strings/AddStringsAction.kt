 ...
 * `update-strings --add-keys --target-file`. The target is always explicit: invoked from a string
 * file's context menu it pre-selects that file's `[[file]]` group; globally it defaults to the
 * first base file. Targeted — never a full sync.
 */
class AddStringsAction :
    LocalizerAction(
        /* text = */ "Add Strings",
