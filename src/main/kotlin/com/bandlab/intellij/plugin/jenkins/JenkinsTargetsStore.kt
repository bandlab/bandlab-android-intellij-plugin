 ...
 * files. So the selection lives here, outside the dialog: open the action on file A, pick some
 * tests, close it; open it on file B, and A's targets are still there — B's just get added.
 *
...
class JenkinsTargetsStore {
    ...
    /**
     * Replaces the stored targets that belong to [classFqns] with [newTargets], leaving targets
     * from every other class untouched. Called whenever the current file's checkbox selection
     * changes, so re-selecting within one file never disturbs what was picked in other files.
     */
